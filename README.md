...
------
## 一、项目简介
谛听（Diting）依托eBPF LSM实现无侵入内核级文件审计，兼顾**静态基线快照比对**与**实时运行时篡改检测**，弥补传统静态扫描无法实时防护、通用监控工具无可信文件基准的短板。
核心能力拆分：
...
------
## 十、开源许可证
本项目源码采用 [Business Source License 1.1](LICENSE)（BSL 1.1）授权，
商标见 [TRADEMARK.md](TRADEMARK.md)。

| 使用场景 | 是否免费 |
|---|---|
| 个人学习、研究、开发测试、内部评估 | ✅ 免费 |
| 提交 Issue / PR 参与贡献 | ✅ 免费 |
| 公司内部生产环境部署使用 | ❌ 需购买商业许可 |
| 集成进对外销售的产品 / 对外提供安全检测服务 | ❌ 需购买商业许可 |
| 发布满 3 年的历史版本 | ✅ 已自动转为 GPLv2+，永久免费 |

商业授权咨询：[kabc005009@163.com]

## 谛听（Diting）是等保三级“安全计算环境”内核层检查的技术补充组件，规则按 GB/T 22239-2019 8.1.4.x 做条款映射，用于辅助测评机构/安全服务商获取运行时证据；其输出不构成法定测评结论，最终合规判定以持证测评机构依据 GB/T 28448-2019 的测评报告为准。